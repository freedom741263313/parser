...
### 2.2 范围
- 在范围：UDP数据解析、规则编辑、枚举管理、导入/导出功能、UDP数据发送和接收解析、样本数据生成与管理、多主题切换。
...
- **日志增强**：
    - **交互**：支持选中日志文本进行复制。
    - **快捷复制**：单条日志提供快捷复制按钮，一键复制Hex码流到剪贴板。
    - **详情展示**：点击日志条目展开详情区域。
        - **Hex格式化**：显示格式化的原始Hex数据，每行16字节，方便阅读。
    - **自动解析**：尝试根据当前已有规则库自动解析消息内容。
        - 若匹配成功，显示协议名称。
        - 显示解析后的字段列表表格，包含字段名、值及解析后的含义/格式化值。
- **模版管理**：
...
### 4.5 用户界面
- **菜单栏**：隐藏系统默认的应用程序菜单栏（如 File, Edit 等），保持界面简洁。
- **导航栏**：左侧侧边栏导航，支持折叠/展开以节省屏幕空间。
- **本地化**：全中文界面，无英文残留。
- **主界面**：输入区、规则编辑区、输出区、发送/接收配置区。
- **对齐**：输入区标题栏与结果区标题栏高度一致，确保视觉整齐。
- **状态保持**：
    - **页面切换**：在不同功能模块（如解析器、调试、规则管理）之间切换时，必须保持各页面的当前状态。
    - **数据持久化**：
        - 解析器页面：输入框内容、解析结果表格、选中的协议等不应被清空。
        - UDP调试页面：日志记录、当前的发送/接收配置、输入框内容不应丢失。
        - 规则/枚举管理页面：当前的编辑状态、搜索条件等应保持不变。
- **主题系统**：
    - **多主题支持**：提供多种预设主题供用户选择，包括：
        - **明亮 (Light)**：默认主题，适合明亮环境。
        - **暗黑 (Dark)**：深色背景，适合低光环境。
        - **护眼 (Eye-care)**：柔和的绿色调，减少视疲劳。
        - **海洋 (Ocean)**：蓝色调，清新风格。
    - **切换方式**：在侧边栏底部提供直观的主题切换器。
    - **全局生效**：主题切换即时应用到所有界面元素（背景、文字、边框等）。
    - **配置持久化**：用户选择的主题会被记住，下次启动应用时自动应用。
